...
Sub VolumeDown(Remote)
	SendSubscriberMessage 1, "Russound." & Remote & ".10.Volume:" &  GetPropertyValue(Remote & ".Selected Zone Name") & ":Down"
	'Dim CurrentVolume
...
Sub VolumeUp(Remote)
	SendSubscriberMessage 1, "Russound." & Remote & ".10.Volume:" &  GetPropertyValue(Remote & ".Selected Zone Name") & ":Up"
	'Dim CurrentVolume
	'CurrentVolume=CInt(GetPropertyValue("Multiroom Audio Settings.Zone " & GetPropertyValue(Remote & ".Selected Zone") & " Volume"))
...
Sub AVOn2 (Source, Zone)
	...
	If ((CInt(GetPropertyValue("Multiroom Audio Settings.Zone " & CStr(ZoneName2ID(Zone)) & " TV")) = 1) And (CInt(GetPropertyValue("Multiroom Audio Settings.Source " & CStr(SourceName2ID(Source)) & " TV")) = 1))  Then
		...
		If CInt(GetPropertyValue("Multiroom Audio Settings.Zone " & CStr(ZoneName2ID(Zone)) & " TV Status")) = 0 Then		
		...
		End if	
	Else
		If Cint(GetPropertyValue("Multiroom Audio Settings.Zone " & CStr(ZoneName2ID(Zone)) & " TV Status")) = 1 Then		
